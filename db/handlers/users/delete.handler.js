...
class DeleteUserHandler {
    ...
    async Delete(currUser, t) {
        ...
        //User is an Organizer, delete their db table rows and images
            ...
            await getEventHandler.FindOrganizerEvents(currUser.id, t)
                .then(async (events) => {
                    console.log("FOUND ORGANIZER EVENTS: " + events.length);
                    //Delete the Organizer's events
                    for (let event of events) {
                        await deleteEventHandler.Delete(event.id, t)
                    ...
                    for (let filename of eventImgFilenames) {
                        ...
                        if (filename != "") {
                            console.log("Event Image Filename exists! Deleting Image from S3 bucket!");
