...
class DeleteEventHandler {
	...
	async DeleteFavouritedBy(eventId, transaction) {
		...
		await FavouritedBy.destroy({ where: { EventId: eventId }, transaction: transaction })
			.then((result) => {
	...
	async DeleteTaggedWith(eventId, transaction) {
		...
		let deleteResult;
		//Delete the tag junctions
		await TaggedWith.destroy({ where: { EventId: eventId }, transaction: transaction })
			.then((result) => {
	...
	async DeleteActs(eventId, transaction) {
		...
		let junctions = await EventAct.findAll({ where: { EventId: eventId }, transaction: transaction });
		//Delete the Event-Act junctions
		await EventAct.destroy({ where: { EventId: eventId }, transaction: transaction })
			.then(async (result) => {
				...
				for (let junc of junctions)
					await Act.destroy({ where: { id: junc.dataValues.ActId }, transaction: transaction })
						.then((result) => {
	...
	async DeleteTicketTypes(eventId, transaction) {
		...
		let junctions = await EventTicket.findAll({ where: { EventId: eventId }, transaction: transaction });
		//Delete the Event-Ticket Type junctions
		await EventTicket.destroy({ where: { EventId: eventId }, transaction: transaction })
			.then(async (result) => {
				console.log("Ticket Type-Event junctions deleted: " + result);

				//Find and delete the Ticket Type rows after junctions were deleted
				for (let junc of junctions)
					await TicketType.destroy({ where: { id: junc.dataValues.TicketTypeId }, transaction: transaction })
						.then((result) => {
	...
	async DeleteEventImage(eventId, transaction) {
		...
		let eventImg = await EventImage.findOne({ where: { EventId: eventId }, transaction: transaction });
		//If event image exists, destroy it
		if (eventImg != null) {
			await EventImage.destroy({ where: { id: eventImg.id }, transaction: transaction })
				.then((result) => {
					console.log("Event Image Deleted: " + result);
					console.log(eventImg);
					//If the event image row has a filename, assign to variable
					if (eventImg.dataValues.filename != null && eventImg.dataValues.filename !== undefined) {
	...
	async DeleteEvent(eventId, transaction) {
		...
		await Event.destroy({ where: { id: eventId }, transaction: transaction })
			.then((result) => {
